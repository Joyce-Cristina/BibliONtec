...
app.post('/cadastrarAluno', (req, res) => {
  const { nome, telefone, email, senha, tipo_usuario_id } = req.body;
  const foto = "temp"; 
  const tiposValidos = [1, 2];
  if (!tiposValidos.includes(Number(tipo_usuario_id))) {
    return res.status(400).json({ error: 'Tipo de usuário inválido.' });
  }

  const sql = `
    INSERT INTO usuario (nome, telefone, email, senha, foto, FK_tipo_usuario_id)
    VALUES (?, ?, ?, ?, ?, ?)
  `;
